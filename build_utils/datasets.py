...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        # load image
        img, _, (h, w) = load_image(self, index)    # h w为预处理后的尺寸
    ...
    if len(labels4):
        ...
        # labels4[:, 1:]为输入数组； 0为限定的最小值；2*s未限定的最大值，out=labels4[:, 1:]为输出数组
...
def random_affine(img, targets=(), degrees=10, translate=.1, scale=.1, shear=10, border=0):
    ...
    # Rotation and Scale
    R = np.eye(3)
    a = random.uniform(-degrees, degrees)   # 生成一个实数，范围在[-degrees, degrees]
    # a += random.choice([-180, -90, 0, 90])  # add 90deg rotations to small rotations
    s = random.uniform(1 - scale, 1 + scale)
    # s = 2 ** random.uniform(-scale, scale)
    # 使用cv2.getRotationMatrix2D()实现图像旋转
    # center：图片的旋转中心
    # angle：旋转角度
...
def letterbox(img: np.ndarray, new_shape=(416, 416), color=(114, 114, 114), auto=True, scale_fill=False, scale_up=True):
    """
    将图片缩放调整到指定大小
    :param img:
    :param new_shape:

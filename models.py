...
def create_modules(modules_defs: list, img_size):
    ...
    output_filters = [3]  # input channels 存储每一层的输出
    ...
    for i, mdef in enumerate(modules_defs):
        ...
        if mdef["type"] == "convolutional":
            ...
            # 记录哪些特征层的输出会被后续的层使用到(可能是特征融合，也可能是拼接)
            ...
            # 修改YOLO层的前一层predict层的偏置
            try:
                j = -1
                bias_ = module_list[j][0].bias  # shape(75,) 索引0对应Sequential中的Conv2d 根据数据集class数变化
                bias = bias_.view(modules.na, -1)  # shape(3, 25)  根据数据集class数变化
                bias[:, 4] += -4.5  # obj   # confidence 置信度
                bias[:, 5:] += math.log(0.6 / (modules.nc - 0.99))  # cls (sigmoid(p) = 1/nc)
                module_list[j][0].bias = torch.nn.Parameter(bias_, requires_grad=bias_.requires_grad) #nn.Parameter
        ...
        module_list.append(modules) # 添加添加每一层的Sequential模块
        output_filters.append(filters)  # 添加每一层的输出

    routs_binary = [False] * len(modules_defs)  # 获取网络结构中需要保存输出给后续层使用的层数
    for i in routs:
        routs_binary[i] = True
...
class YOLOLayer(nn.Module):
    ...
    def __init__(self, anchors, nc, img_size, stride):
        super(YOLOLayer, self).__init__()   # 继承父类Module的所有属性

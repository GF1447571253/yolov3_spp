...
def train(hyp):
    ...
    # python是跨平台的。在Windows上，文件的路径分隔符是'\'，在Linux上是'/'  使用os.sep的话，就不用考虑这个了，os.sep根据你所处的平台，自动采用相应的分隔符号。
    ...
    if multi_scale:
        imgsz_min = opt.img_size // 1.5 # 取整除，向下取整
    ...
    hyp["cls"] *= nc / 80  # update coco-tuned hyp['cls'] to current dataset 根据自己数据集的类别数设置分类损失的系数
    hyp["obj"] *= imgsz_test / 320  # obj loss gain (*=img_size/320 if img_size != 320)
    ...
    # plot loss and learning_rate curve
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='weights/yolov3spp-199.pt',
    ...
    # 检查文件是否存在
    opt.cfg = check_file(opt.cfg)   # YOLO的网络配置文件
    opt.data = check_file(opt.data) # 数据集
    opt.hyp = check_file(opt.hyp)   # 超参数
    # Namespace(batch_size=4, cache_images=False, cfg='cfg/my_yolov3.cfg', data='data/my_data.data',
    # device='cuda:0', epochs=30, freeze_layers=False, hyp='cfg/hyp.yaml', img_size=512, multi_scale=True, name='',
    # notest=False, rect=False, savebest=False, single_cls=False, weights='weights/yolov3-spp-ultralytics-512.pt')
    print(opt)
    # 将hyp.yaml文件下的超参数赋予hyp
